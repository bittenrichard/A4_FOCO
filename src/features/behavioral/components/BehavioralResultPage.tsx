...
const ErrorState: React.FC<{ message: string, onRetry: () => void }> = ({ message, onRetry }) => (
  <div className="text-center py-20 bg-red-50 p-8 rounded-lg">
    ...
    <button onClick={onRetry} className="mt-6 px-6 py-2 bg-red-600 text-white font-semibold rounded-md hover:bg-red-700 transition-colors">
      Tentar Novamente
    </button>
  </div>
);
...
const InfoCard: React.FC<{ icon: React.ElementType, title: string, content: string | null }> = ({ icon: Icon, title, content }) => (
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
            <Icon size={20} className="mr-3 text-indigo-600"/> 
            {title}
        </h3>
        <p className="text-gray-700 leading-relaxed whitespace-pre-wrap">
            {content || 'Nenhuma informação gerada.'}
        </p>
    </div>
);
...
const BehavioralResultPage: React.FC<BehavioralResultPageProps> = ({ testId, onBack }) => {
    ...
    if (!result) return <ErrorState message="Não foi possível carregar os dados." onRetry={fetchResult} />;
    ...
    return (
        <div className="fade-in max-w-6xl mx-auto space-y-8">
            <button onClick={onBack} className="text-indigo-600 hover:underline font-semibold">
                &larr; Voltar para a lista de testes
            </button>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start">
                    <div>
                        ...
                        <div className="flex items-center text-gray-500 mt-2">
                            ...
                            <span>Teste respondido em: {result.data_de_resposta ? format(new Date(result.data_de_resposta), "dd 'de' MMMM 'de' yyyy", { locale: ptBR }) : 'N/A'}</span>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <InfoCard icon={BarChart2} title="Indicadores Situacionais" content={result.indicadores} />
                </div>
