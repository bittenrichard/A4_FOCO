...
interface BehavioralResultPageProps {
  testId: number;
  onBack: () => void;
}
...
const LoadingState: React.FC<{ message: string }> = ({ message }) => (
    <div className="text-center py-20">
        ...
        <p className="mt-2 text-gray-600">Isso pode levar alguns instantes.</p>
    </div>
);
...
const BehavioralResultPage: React.FC<BehavioralResultPageProps> = ({ testId, onBack }) => {
    ...
    const fetchResult = useCallback(async () => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(data.error || 'Falha ao buscar o resultado do teste.');
            }
            const { data } = await response.json();
            setResult(data);
            return data;
    ...
    useEffect(() => {
        ...
        const loadAndPoll = async () => {
            ...
            if (initialResult?.status === 'Processando') {
                intervalId = setInterval(async () => {
                    const currentResult = await fetchResult();
                    if (currentResult?.status === 'Concluído' || currentResult?.status === 'Erro') {
    ...
    if (isLoading) {
        return <LoadingState message="Buscando resultado do teste..." />;
    }

    if (error) {
        return <ErrorState message={error} onRetry={fetchResult} />;
    }

    if (!result) {
        return <ErrorState message="Não foi possível carregar os dados do resultado." onRetry={fetchResult} />;
    }
    
    if (result.status === 'Processando') {
        return <LoadingState message="Aguardando análise da IA..." />;
    }
        ...
        <div className="fade-in max-w-5xl mx-auto space-y-8">
            ...
            <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200">
                <div className="flex flex-col sm:flex-row justify-between items-start">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">{result.candidato[0]?.value || 'Candidato'}</h1>
                        <div className="flex items-center text-gray-500 mt-2">
                            <Calendar size={16} className="mr-2" />
                            <span>Teste respondido em: {format(new Date(result.data_de_resposta), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}</span>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                     <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                        ...
                        <p className="text-gray-700 leading-relaxed">{result.resumo_perfil || 'Nenhum resumo gerado.'}</p>
                    </div>
                     <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                        <h3 className="text-lg font-semibold text-gray-800 mb-4">Habilidades Comuns</h3>
                        <p className="text-gray-700 leading-relaxed">{result.habilidades_comuns || 'Nenhuma habilidade comum gerada.'}</p>
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                        <h3 className="text-lg font-semibold text-gray-800 mb-4">Indicadores</h3>
                        <p className="text-gray-700 leading-relaxed">{result.indicadores || 'Nenhum indicador gerado.'}</p>
                    </div>
                </div>
