    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 sm:p-8 flex items-center justify-center">
            <div className="max-w-4xl w-full mx-auto bg-white p-6 sm:p-8 rounded-lg shadow-lg">
                ...
                <div className="my-6"><ProgressBar progress={step === 1 ? (currentAnswers.length / SELECTIONS_LIMIT) * 50 : 50 + (currentAnswers.length / SELECTIONS_LIMIT) * 50} /></div>
                ...
                <div className="bg-gray-50 p-6 rounded-lg border">
                    ...
                    <p className="mt-4 font-bold text-indigo-700">Selecione exatamente {SELECTIONS_LIMIT} opções. ({currentAnswers.length}/{SELECTIONS_LIMIT})</p>
                </div>
                ...
                <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {adjectives.map(adj => <AdjectiveButton key={adj} adjective={adj} isSelected={currentAnswers.includes(adj)} isDisabled={currentAnswers.length >= SELECTIONS_LIMIT} onClick={() => handleSelect(adj)} />)}
                </div>

                <div className="mt-10 flex justify-end items-center">
                    {step === 1 ? (
                        <button onClick={handleNextStep} disabled={currentAnswers.length !== SELECTIONS_LIMIT} className="px-8 py-3 bg-indigo-600 text-white font-semibold rounded-md hover:bg-indigo-700 transition-colors disabled:opacity-50">Próximo Passo</button>
                    ) : (
                        <button onClick={handleSubmit} disabled={isSubmitting || currentAnswers.length !== SELECTIONS_LIMIT} className="px-8 py-3 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition-colors disabled:opacity-50">{isSubmitting ? <Loader2 className="animate-spin" /> : 'Finalizar Teste'}</button>
                    )}
                </div>
            </div>
        </div>
